...
test('it loads the default theme when a theme has not been saved', () => {
  setup(null);
  expect(screen.getByLabelText("webog")).toHaveAttribute("aria-checked", "true");
...
test('it loads a saved theme', () => {
  ...
  expect(screen.getByLabelText("webog")).toHaveAttribute("aria-checked", "false");
  expect(screen.getByLabelText("portland")).toHaveAttribute("aria-checked", "true");
});
